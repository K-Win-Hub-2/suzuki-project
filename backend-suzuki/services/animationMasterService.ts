...
import {
  AnimationMaster,
  AnimationMasterModel,
} from "../models/animationMaster";
...
class AnimationMasterClass {
  ...
  async listAll(datas: FilterQuery<AnimationMaster>) {
    ...
    return successResponse({
      statusCode: 200,
      message: "These are all Animation datas",
      data: data,
    });
  }
  ...
  async create(files: any, data: AnimationMaster) {
    ...
    return successResponse({
      statusCode: 200,
      message: "Animation Data Created successfully",
      data: result,
    });
  }
  ...
  async readById(id: mongoose.Types.ObjectId) {
    ...
    return successResponse({
      statusCode: 200,
      message: "This is Animation Data by id",
      data: result,
    });
  }
  ...
  async updateById(
    ...
    datas: AnimationMaster
  ) {
    console.log("files", files);

    if (files && files.length > 0) {
      datas.url = files[0].location;
    }

    const result = await AnimationMasterModel.findByIdAndUpdate(id, datas, {
      new: true,
    });

    return successResponse({
      statusCode: 200,
      message: "Animation Data Updated successfully",
      data: result,
    });
  }

  async delete(id: mongoose.Types.ObjectId) {
    const result = await AnimationMasterModel.findByIdAndUpdate(
      id,
      { isDeleted: true },
      { new: true }
    );
    return successResponse({
      statusCode: 200,
      message: "Animation Data deleted successfully",
      data: result,
    });
  }
}

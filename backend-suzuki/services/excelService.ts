...
const excelImport = async (file: any) => {
        ...
        for(let i= 0; i < data.length; i++){
            ...
            let query: Partial<{ isDeleted: boolean, partNumber: string, dealerId: mongoose.Types.ObjectId }> = { isDeleted: false }
            partNumber ? query.partNumber = partNumber : ""
            dealerResult ? query.dealerId  = new mongoose.Types.ObjectId(dealerResult._id as mongoose.Types.ObjectId) : ""

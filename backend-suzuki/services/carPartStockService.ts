...
class CarPartStockClass {
    ...
    async create(file: any,data: CarPartStockModel){
        ...
        if(file){
         ...
         //call dealer account
         const dealerAccount = await AdminUsers.find({ isDeleted: false, isSuperAdmin: false})
         ...
         return successResponse({ statusCode:200, message: "Car Part Stock Created successfully", data: results})
    }
    ...
    async updateAll(file: any, partTitleId: mongoose.Types.ObjectId, partNumber: string,  datas: CarPartStockModel){
        ...
        if(file){
         ...
         if(findAllStock.length > 0) {
            for(let i=0;  i < findAllStock.length; i++){
                const carStock: CarPartStockModel   = await CarPartStockModels.findByIdAndUpdate(findAllStock[i]._id, datas, { new: true }) as  CarPartStockModel 
                result.push(carStock)
            }
         }
         return successResponse({ statusCode:200, message: "Car Part Stock Updated By All successfully", data: result})
    }
